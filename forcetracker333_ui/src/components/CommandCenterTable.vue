        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Command Centers</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalCommandCenters = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCommandCenters">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add CommandCenter</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CommandCenterId" type="text" placeholder="Enter CommandCenterId" v-model="commandCenterToAdd.commandCenterId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="commandCenterToAdd.name"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="commandCenters" :row-key="record => record.CommandCenterId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CommandCenterPictureView :commandCenters="commandCenters" />
          </div>
...
import CommandCenterPictureView from './CommandCenterPictureView.vue';


const commandCentersColumns = [
  "commandCenterId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    CommandCenterPictureView
  },

  data() {
    return {
      modalCommandCenters: false,
        isTableView: true,

      columns: [
        {
          title: 'Command Center Id',
		dataIndex: 'commandCenterId',
          visible: true,
          scopedSlots: { customRender: 'commandCenterId' },
          sorter: true
          //sorter: (a, b) => a.commandCenterId - b.commandCenterId,
          //sorter: (a, b) => a.commandCenterId.localeCompare(b.commandCenterId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} commandCenters`,
      },
      ...
      commandCentersTable: {
        columns: [...commandCentersColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'commandCenterId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCommandCentersTable() {
      ...
      let commandCentersTableData = [];
      for (let i = 0; i < this.commandCenters.length; i++) {
        commandCentersTableData.push({
          id: i,
          commandCenterId: this.commandCenters[i].commandCenterId,
          year: this.commandCenters[i].year,
          date: this.commandCenters[i].date,
          competitionId: this.commandCenters[i].competitionId,
          commandCenterId: this.commandCenters[i].commandCenterId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-command-centers',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCommandCentersChanged', this.searchQuery);
		//this.renderCommandCentersTable();
    },
	...
	async handleAddSubmitted() {
      this.modalCommandCenters = false;

      const currentDate = new Date().getTime();
      this.commandCenterToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.commandCenterToAdd);
      console.log(jsonData);
      
      const res = await CommandCenterService.addCommandCenter(jsonData);
