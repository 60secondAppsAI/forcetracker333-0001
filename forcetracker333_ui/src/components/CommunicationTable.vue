        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalCommunications = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCommunications">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Communication</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CommunicationId" type="text" placeholder="Enter CommunicationId" v-model="communicationToAdd.communicationId"></base-input>
  <base-input label="Type" type="text" placeholder="Enter Type" v-model="communicationToAdd.type"></base-input>
  <base-input label="Content" type="text" placeholder="Enter Content" v-model="communicationToAdd.content"></base-input>
  <base-input label="DateTime" type="text" placeholder="Enter DateTime" v-model="communicationToAdd.dateTime"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="communications" :row-key="record => record.CommunicationId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CommunicationPictureView :communications="communications" />
          </div>
...
import CommunicationPictureView from './CommunicationPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    CommunicationPictureView
  },

  data() {
    return {
      modalCommunications: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'communicationId',
          visible: true,
          scopedSlots: { customRender: 'communicationId' },
          sorter: true
          //sorter: (a, b) => a.communicationId - b.communicationId,
          //sorter: (a, b) => a.communicationId.localeCompare(b.communicationId),
        },
        {
          title: 'Type',
		dataIndex: 'type',
      ...
      sortBy: 'communicationId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCommunicationsTable() {
      ...
      let communicationsTableData = [];
      for (let i = 0; i < this.communications.length; i++) {
        communicationsTableData.push({
          id: i,
          communicationId: this.communications[i].communicationId,
          year: this.communications[i].year,
          date: this.communications[i].date,
          competitionId: this.communications[i].competitionId,
          communicationId: this.communications[i].communicationId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-communications',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCommunicationsChanged', this.searchQuery);
		//this.renderCommunicationsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalCommunications = false;

      const currentDate = new Date().getTime();
      this.communicationToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.communicationToAdd);
      console.log(jsonData);
      
      const res = await CommunicationService.addCommunication(jsonData);
