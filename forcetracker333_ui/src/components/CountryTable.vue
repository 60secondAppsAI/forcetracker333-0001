        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Countrys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalCountrys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCountrys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Country</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CountryId" type="text" placeholder="Enter CountryId" v-model="countryToAdd.countryId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="countryToAdd.name"></base-input>
  <base-input label="Code" type="text" placeholder="Enter Code" v-model="countryToAdd.code"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="countrys" :row-key="record => record.CountryId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CountryPictureView :countrys="countrys" />
          </div>
...
import CountryPictureView from './CountryPictureView.vue';


const countrysColumns = [
  "countryId",
  "year",
  "date",
  "competitionId",
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'countryId',
          ...
          sorter: true
          //sorter: (a, b) => a.countryId - b.countryId,
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} countrys`,
      },
      ...
      countrysTable: {
        columns: [...countrysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'countryId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCountrysTable() {
      ...
      let countrysTableData = [];
      for (let i = 0; i < this.countrys.length; i++) {
        countrysTableData.push({
          id: i,
          countryId: this.countrys[i].countryId,
          year: this.countrys[i].year,
          date: this.countrys[i].date,
          competitionId: this.countrys[i].competitionId,
          countryId: this.countrys[i].countryId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-countrys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCountrysChanged', this.searchQuery);
		//this.renderCountrysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalCountrys = false;

      const currentDate = new Date().getTime();
      this.countryToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.countryToAdd);
      console.log(jsonData);
      
      const res = await CountryService.addCountry(jsonData);
