        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalIntelligenceReports = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalIntelligenceReports">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add IntelligenceReport</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="IntelligenceReportId" type="text" placeholder="Enter IntelligenceReportId" v-model="intelligenceReportToAdd.intelligenceReportId"></base-input>
  <base-input label="Summary" type="text" placeholder="Enter Summary" v-model="intelligenceReportToAdd.summary"></base-input>
  <base-input label="Date" type="text" placeholder="Enter Date" v-model="intelligenceReportToAdd.date"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="intelligenceReports" :row-key="record => record.IntelligenceReportId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <IntelligenceReportPictureView :intelligenceReports="intelligenceReports" />
          </div>
...
import IntelligenceReportPictureView from './IntelligenceReportPictureView.vue';


const intelligenceReportsColumns = [
...
export default {
  ...
  components: {
    ...
    Pagination,
    IntelligenceReportPictureView
  },

  data() {
    return {
      modalIntelligenceReports: false,
        isTableView: true,

      columns: [
        {
          title: 'Intelligence Report Id',
		dataIndex: 'intelligenceReportId',
          visible: true,
          scopedSlots: { customRender: 'intelligenceReportId' },
          sorter: true
          //sorter: (a, b) => a.intelligenceReportId - b.intelligenceReportId,
          //sorter: (a, b) => a.intelligenceReportId.localeCompare(b.intelligenceReportId),
        },
        {
          title: 'Summary',
		dataIndex: 'summary',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} intelligenceReports`,
      },

      intelligenceReports: [],
      intelligenceReportToAdd: {},

      intelligenceReportsTable: {
        columns: [...intelligenceReportsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'intelligenceReportId',           // Column to sort by
  ...
  methods: {
    ...
    async renderIntelligenceReportsTable() {
      ...
      let intelligenceReportsTableData = [];
      for (let i = 0; i < this.intelligenceReports.length; i++) {
        intelligenceReportsTableData.push({
          id: i,
          intelligenceReportId: this.intelligenceReports[i].intelligenceReportId,
          year: this.intelligenceReports[i].year,
          date: this.intelligenceReports[i].date,
          competitionId: this.intelligenceReports[i].competitionId,
          intelligenceReportId: this.intelligenceReports[i].intelligenceReportId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-intelligence-reports',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForIntelligenceReportsChanged', this.searchQuery);
		//this.renderIntelligenceReportsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalIntelligenceReports = false;

      const currentDate = new Date().getTime();
      this.intelligenceReportToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.intelligenceReportToAdd);
      console.log(jsonData);
      
      const res = await IntelligenceReportService.addIntelligenceReport(jsonData);
