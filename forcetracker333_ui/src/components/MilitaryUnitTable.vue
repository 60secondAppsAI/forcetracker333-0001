        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Military Units</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalMilitaryUnits = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMilitaryUnits">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add MilitaryUnit</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MilitaryUnitId" type="text" placeholder="Enter MilitaryUnitId" v-model="militaryUnitToAdd.militaryUnitId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="militaryUnitToAdd.name"></base-input>
  <base-input label="Type" type="text" placeholder="Enter Type" v-model="militaryUnitToAdd.type"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="militaryUnits" :row-key="record => record.MilitaryUnitId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MilitaryUnitPictureView :militaryUnits="militaryUnits" />
          </div>
...
import MilitaryUnitPictureView from './MilitaryUnitPictureView.vue';


const militaryUnitsColumns = [
  "militaryUnitId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    MilitaryUnitPictureView
  },

  data() {
    return {
      modalMilitaryUnits: false,
        isTableView: true,

      columns: [
        {
          title: 'Military Unit Id',
		dataIndex: 'militaryUnitId',
          visible: true,
          scopedSlots: { customRender: 'militaryUnitId' },
          sorter: true
          //sorter: (a, b) => a.militaryUnitId - b.militaryUnitId,
          //sorter: (a, b) => a.militaryUnitId.localeCompare(b.militaryUnitId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} militaryUnits`,
      },
      ...
      militaryUnitsTable: {
        columns: [...militaryUnitsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'militaryUnitId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMilitaryUnitsTable() {
      ...
      let militaryUnitsTableData = [];
      for (let i = 0; i < this.militaryUnits.length; i++) {
        militaryUnitsTableData.push({
          id: i,
          militaryUnitId: this.militaryUnits[i].militaryUnitId,
          year: this.militaryUnits[i].year,
          date: this.militaryUnits[i].date,
          competitionId: this.militaryUnits[i].competitionId,
          militaryUnitId: this.militaryUnits[i].militaryUnitId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-military-units',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMilitaryUnitsChanged', this.searchQuery);
		//this.renderMilitaryUnitsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalMilitaryUnits = false;

      const currentDate = new Date().getTime();
      this.militaryUnitToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.militaryUnitToAdd);
      console.log(jsonData);
      
      const res = await MilitaryUnitService.addMilitaryUnit(jsonData);
