        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalMissions = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMissions">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Mission</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MissionId" type="text" placeholder="Enter MissionId" v-model="missionToAdd.missionId"></base-input>
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="missionToAdd.name"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="missionToAdd.description"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="missions" :row-key="record => record.MissionId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MissionPictureView :missions="missions" />
          </div>
...
import MissionPictureView from './MissionPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'missionId',
          visible: true,
          scopedSlots: { customRender: 'missionId' },
          sorter: true
          //sorter: (a, b) => a.missionId - b.missionId,
          //sorter: (a, b) => a.missionId.localeCompare(b.missionId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      sortBy: 'missionId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMissionsTable() {
      ...
      let missionsTableData = [];
      for (let i = 0; i < this.missions.length; i++) {
        missionsTableData.push({
          id: i,
          missionId: this.missions[i].missionId,
          year: this.missions[i].year,
          date: this.missions[i].date,
          competitionId: this.missions[i].competitionId,
          missionId: this.missions[i].missionId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-missions',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMissionsChanged', this.searchQuery);
		//this.renderMissionsTable();
    },
	...
	async handleAddSubmitted() {
      ...
      this.missionToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.missionToAdd);
      console.log(jsonData);
      
      const res = await MissionService.addMission(jsonData);
