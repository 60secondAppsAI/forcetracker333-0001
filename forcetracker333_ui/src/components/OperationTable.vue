...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalOperations = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalOperations">
                <template slot="header">
  ...
  <base-input label="EndDate" type="text" placeholder="Enter EndDate" v-model="operationToAdd.endDate"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="operations" :row-key="record => record.OperationId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <OperationPictureView :operations="operations" />
          </div>
...
import OperationPictureView from './OperationPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    OperationPictureView
  },

  data() {
    return {
      modalOperations: false,
        isTableView: true,
      ...
      sortBy: 'operationId',           // Column to sort by
  ...
  methods: {
    ...
    async renderOperationsTable() {
      ...
      let operationsTableData = [];
      for (let i = 0; i < this.operations.length; i++) {
        operationsTableData.push({
          id: i,
          operationId: this.operations[i].operationId,
          year: this.operations[i].year,
          date: this.operations[i].date,
          competitionId: this.operations[i].competitionId,
          operationId: this.operations[i].operationId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-operations',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForOperationsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.operationToAdd.created = currentDate;
