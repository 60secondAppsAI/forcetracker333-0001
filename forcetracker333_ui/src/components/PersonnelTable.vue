        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalPersonnels = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalPersonnels">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Personnel</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="PersonnelId" type="text" placeholder="Enter PersonnelId" v-model="personnelToAdd.personnelId"></base-input>
  <base-input label="Assignment" type="text" placeholder="Enter Assignment" v-model="personnelToAdd.assignment"></base-input>
  <base-input label="DateAssigned" type="text" placeholder="Enter DateAssigned" v-model="personnelToAdd.dateAssigned"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="personnels" :row-key="record => record.PersonnelId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <PersonnelPictureView :personnels="personnels" />
          </div>
...
import PersonnelPictureView from './PersonnelPictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    PersonnelPictureView
  },

  data() {
    return {
      modalPersonnels: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'personnelId',
          ...
          sorter: true
          //sorter: (a, b) => a.personnelId - b.personnelId,
          //sorter: (a, b) => a.personnelId.localeCompare(b.personnelId),
        },
        {
          title: 'Assignment',
		dataIndex: 'assignment',
          ...
          sorter: true
          //sorter: (a, b) => a.assignment - b.assignment,
		...
		dataIndex: 'dateAssigned',
          visible: true,
          scopedSlots: { customRender: 'dateAssigned' },
          sorter: true
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} personnels`,
      },
      ...
      sortBy: 'personnelId',           // Column to sort by
  ...
  methods: {
    ...
    async renderPersonnelsTable() {
      ...
      let personnelsTableData = [];
      for (let i = 0; i < this.personnels.length; i++) {
        personnelsTableData.push({
          id: i,
          personnelId: this.personnels[i].personnelId,
          year: this.personnels[i].year,
          date: this.personnels[i].date,
          competitionId: this.personnels[i].competitionId,
          personnelId: this.personnels[i].personnelId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-personnels',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForPersonnelsChanged', this.searchQuery);
		//this.renderPersonnelsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalPersonnels = false;

      const currentDate = new Date().getTime();
      this.personnelToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.personnelToAdd);
      console.log(jsonData);
      
      const res = await PersonnelService.addPersonnel(jsonData);
