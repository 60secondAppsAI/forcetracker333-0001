        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalRiskAssessments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRiskAssessments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add RiskAssessment</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="RiskAssessmentId" type="text" placeholder="Enter RiskAssessmentId" v-model="riskAssessmentToAdd.riskAssessmentId"></base-input>
  <base-input label="Level" type="text" placeholder="Enter Level" v-model="riskAssessmentToAdd.level"></base-input>
  <base-input label="Date" type="text" placeholder="Enter Date" v-model="riskAssessmentToAdd.date"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="riskAssessments" :row-key="record => record.RiskAssessmentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <RiskAssessmentPictureView :riskAssessments="riskAssessments" />
          </div>
...
import RiskAssessmentPictureView from './RiskAssessmentPictureView.vue';


const riskAssessmentsColumns = [
  "riskAssessmentId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    RiskAssessmentPictureView
  },

  data() {
    return {
      modalRiskAssessments: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'riskAssessmentId',
          visible: true,
          scopedSlots: { customRender: 'riskAssessmentId' },
          sorter: true
          //sorter: (a, b) => a.riskAssessmentId - b.riskAssessmentId,
          //sorter: (a, b) => a.riskAssessmentId.localeCompare(b.riskAssessmentId),
        },
        {
          title: 'Level',
		dataIndex: 'level',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} riskAssessments`,
      },
      ...
      sortBy: 'riskAssessmentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderRiskAssessmentsTable() {
      ...
      let riskAssessmentsTableData = [];
      for (let i = 0; i < this.riskAssessments.length; i++) {
        riskAssessmentsTableData.push({
          id: i,
          riskAssessmentId: this.riskAssessments[i].riskAssessmentId,
          year: this.riskAssessments[i].year,
          date: this.riskAssessments[i].date,
          competitionId: this.riskAssessments[i].competitionId,
          riskAssessmentId: this.riskAssessments[i].riskAssessmentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-risk-assessments',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRiskAssessmentsChanged', this.searchQuery);
		//this.renderRiskAssessmentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalRiskAssessments = false;

      const currentDate = new Date().getTime();
      this.riskAssessmentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.riskAssessmentToAdd);
      console.log(jsonData);
      
      const res = await RiskAssessmentService.addRiskAssessment(jsonData);
