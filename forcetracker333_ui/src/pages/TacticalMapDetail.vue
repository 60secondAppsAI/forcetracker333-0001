					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateTacticalMap()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div id="app">
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2TacticalMaps = true">Add</base-button>
								              <modal :show.sync="modal2TacticalMaps">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Tactical Map Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="TacticalMapId" type="text" placeholder="Enter TacticalMapId" v-model="tacticalMapDetails.tacticalMapId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Region</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Region" type="text" placeholder="Enter Region" v-model="tacticalMapDetails.region"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Date Updated</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="tacticalMapDetails.dateUpdated"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      tacticalMapToAdd: {},
      modal2TacticalMaps: false,
      tacticalMapDetails: null,
  ...
  methods: {
    ...
    async updateTacticalMap() {

      const jsonData = JSON.stringify(this.tacticalMapDetails);
      const res = await TacticalMapService.update(jsonData);
...
//        this.getAllTacticalMaps();
      }
    },


    async getTacticalMapDetails() {
      try {
			let response = await TacticalMapService.get(this.tacticalMapId);
			this.tacticalMapDetails = response.data;
        ...
        console.error('Error fetching tacticalMap details:', error);
      }
    },
